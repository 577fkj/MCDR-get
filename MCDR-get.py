...
def get_list():
    ...
    if config['source'] == 1:
        res = urllib.request.Request(url='https://raw.githubusercontent.com/577fkj/MCDR-get/master/list.json', headers=headers)
...
def get_lang():
    ...
    if config['source'] == 1:
        if config['language'] == 'zh-cn':
            res = urllib.request.Request(url='https://raw.githubusercontent.com/577fkj/MCDR-get/master/zh-cn.yml', headers=headers)
        else:
            res = urllib.request.Request(url='https://raw.githubusercontent.com/577fkj/MCDR-get/master/en-us.yml', headers=headers)
        return str(urlopen(res).read(), 'utf-8')
    else:
        if config['language'] == 'zh-cn':
...
def send_config(server, info):
    ...
    ConfigMessage = RTextList(
        ...
        get_text('\n      §6' + lang['downsource'] + ' '),
        get_text('§r[§bGitHub§r]', '国外源', Prefix + ' config source 2'),
        get_text(' '),
        get_text('§r[§bGitee§r]', '国内源', Prefix + ' config source 1'),
        get_text('\n      §6' + lang['jindutiao'] + ' '),
